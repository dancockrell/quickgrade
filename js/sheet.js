...
function layoutTest(test) {
  ...
  var perPage = questionsOnSheet(test) ? rowsPerCol() : mcPerPage(choices);
  var rows = rowsPerCol(), cols = questionsOnSheet(test) ? 1 : colsPerPage(choices);
...
function renderPage(test, pages, pageIdx, who) {
  ...
  /* 0.50in, not 0.62: the wider box reached under the first page bubble.
   * The word itself is about 0.30in, so nothing is lost. */
